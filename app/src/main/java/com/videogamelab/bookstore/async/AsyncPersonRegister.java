...
public class AsyncPersonRegister extends AsyncTask<Person, Void, Person> {

    private FragmentCache cache;

    private BookstoreFault fault;

    public AsyncPersonRegister(FragmentCache cache) {
        this.cache = cache;
        fault = null;

        cache.progress.setMessage("Registering account ...");
    }
    ...
    protected Person doInBackground(Person... params) {
        ...
        try {
            if ((Integer) person.getProperty(Person.ID) < 1) {
                Person result = service.createPerson(
                        (String) person.getProperty(Person.USERNAME),
                        (String) person.getProperty(Person.PASSWORD),
                        (String) person.getProperty(Person.FIRST_NAME),
                        (String) person.getProperty(Person.LAST_NAME));

                person.setProperty(Person.ID, result.getProperty(Person.ID));
            }
    ...
    protected void onPostExecute(Person result) {
        ...
        if (fault == null) {
            cache.sendNotify("Account registered successfully!");
            ...
            if (fault.getCode().compareTo(BookstoreFault.ERROR_CLIENT) == 0) {
                ...
                cache.showDialog("Failed to register account!", AsyncPersonRegister.class, result);
